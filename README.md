...
2. Get Spells
	...
	2. Select spells either by using list filters ![btn-filter](assets/btn-filter.png) or pressing `P` on individual spells to pin them (pinned list will be shown instead of the filtered list)
	3. Open the Table View ![btn-table-view](assets/btn-table-view.png) and click Download CSV ![btn-download-csv](assets/btn-download-csv.png)
3. Run options
...
4. Open `out/spell_cards.html` and hit `Ctrl+P` (`Cmd+P` on Mac):
		...
		Scale:                  100%
		Background graphics:    Enabled (to show color)
